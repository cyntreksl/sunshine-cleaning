...
import { breadcrumbNode, organizationId, webPageNode } from "@/lib/structured-data";
...
export async function generateMetadata({ params }: PageProps<"/guides/[slug]">): Promise<Metadata> {
  ...
  return { title: guide.title, description: guide.socialDescription, alternates: { canonical: `/guides/${guide.slug}/` }, openGraph: { type: "article", title: guide.title, description: guide.socialDescription, url: `/guides/${guide.slug}/`, publishedTime: guide.published, modifiedTime: guide.updated, images: [{ url: "/images/social-card.jpg", width: 1200, height: 630, alt: "Sunshine Cleaning in York" }] } };
}
...
export default async function GuidePage({ params }: PageProps<"/guides/[slug]">) {
  ...
  return <main id="main-content"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Guides", href: "/guides/" }, { label: guide.title }]} /><article className="guide-article"><header className="guide-header page-shell" data-reveal><p className="eyebrow">Sunshine guide</p><h1>{guide.title}</h1><p>{guide.summary}</p><div><time dateTime={guide.published}>Published {new Date(`${guide.published}T12:00:00Z`).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric", timeZone: "UTC" })}</time><span>{guide.readTime}</span></div></header><div className="guide-layout page-shell"><aside><p>In this guide</p><span>{guide.summary}</span>{service ? <Link className="button button-small" href={`/services/${service.slug}/`}>View {service.shortName.toLowerCase()}</Link> : null}</aside><div className="mdx-content"><Body /></div></div></article><ContactBand /><JsonLd data={[
    { ...webPageNode({ url, name: guide.title, description: guide.socialDescription }), mainEntity: { "@id": articleId } },
    { "@context": "https://schema.org", "@type": "Article", "@id": articleId, headline: guide.title, description: guide.socialDescription, datePublished: guide.published, dateModified: guide.updated, mainEntityOfPage: { "@id": `${url}#webpage` }, author: { "@id": organizationId }, publisher: { "@id": organizationId }, image: `${siteConfig.canonicalUrl}/images/social-card.jpg`, inLanguage: "en-GB" },
    breadcrumbNode([
      { name: "Home", url: `${siteConfig.canonicalUrl}/` },
      { name: "Guides", url: `${siteConfig.canonicalUrl}/guides/` },
